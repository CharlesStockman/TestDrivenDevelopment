...
public class StringCalculator {
    ...
    private List<Integer> retrieveNumbers(String numbers) throws IOException {
        ...
        if ( numbers.length() > 1 && ( numbers.endsWith(",") || numbers.startsWith(","))) {
            throw new InvalidParameterException("Parameters cannot start or end with a delimiter");
        }

        Pattern pattern = Pattern.compile("[\n,]{2,}");
        if ( pattern.matcher(numbers).find()) {
            throw new InvalidParameterException("Delimiters must be separated by one delimiter only such as comma or newline");
        }
